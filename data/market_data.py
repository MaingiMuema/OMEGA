...
class MarketData:
    ...
    async def update_market_data(self, symbol: str) -> None:
        try:
            klines = self.exchange_client.get_klines(symbol, settings.TIMEFRAME, limit=10000)
            if not klines:
    ...
    async def update_all_market_data(self):
        ...
        for symbol in exchange_info['symbols']:
            if symbol['status'] == 'TRADING':
                ticker = self.exchange_client.get_symbol_ticker(symbol['symbol'])
    ...
    async def get_historical_data(self, symbol: str, interval: str, limit: int = 10000) -> pd.DataFrame:
        try:
            klines = self.exchange_client.get_klines(symbol, interval, limit=limit)
            df = pd.DataFrame(klines, columns=['timestamp', 'open', 'high', 'low', 'close', 'volume', 'close_time', 'quote_asset_volume', 'number_of_trades', 'taker_buy_base_asset_volume', 'taker_buy_quote_asset_volume', 'ignore'])
            df['timestamp'] = pd.to_datetime(df['timestamp'], unit='ms')
            df.set_index('timestamp', inplace=True)
            df = df.astype({'open': float, 'high': float, 'low': float, 'close': float, 'volume': float})
    ...
    async def get_klines(self, symbol: str, interval: str, limit: int = 10000) -> List[List]:
        try:
            klines = self.exchange_client.get_klines(symbol=symbol, interval=interval, limit=limit)
            return klines
        except Exception as e:
